                    ...
                    <div class="col-md-5">
                        <select name="products[0][id]" class="form-control product-select">
                            ...
                            @foreach($products as $product)
                                <option value="{{ $product->id }}" data-price="{{ $product->price }}">{{ $product->name }} - Rp {{ number_format($product->price, 2) }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-3">
    ...
    <script>
        $(document).ready(function() {
            $('#orders-table').DataTable({
                ...
                order: [[6, 'desc']], // Mengurutkan berdasarkan tanggal pesanan
                pageLength: 10,
                responsive: true,
                searching: true,
                lengthChange: true,
                footerCallback: function (row, data, start, end, display) {
                    var api = this.api();
                    // Menghitung total untuk halaman saat ini
                    var total = api
